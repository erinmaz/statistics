...
Let's change things up a tiny bit, and imagine we only had 3 participants in total in the experiment. And, that each participant contributed data to the three levels of the independent variable, A, B, and C. Before we called the IV `groups`, because there were different groups of participants. Let's change that to `conditions`, because now the same group of participants participates in all three conditions. Here's the new table for a within-participants (repeated measures) version of this experiment:

```{r}
scores <- c(20,11,2,6,2,7,2,11,2)
conditions <- as.character(rep(c("A","B","C"), each=3))
participants <-rep(1:3,3)
df<-data.frame(participants,conditions,scores)
...
diff_squared <-diff^2
df<-data.frame(participants,conditions,scores,diff, diff_squared)
...
diff_squared <-diff^2
df<-data.frame(participants,conditions,scores,means,diff, diff_squared)
df$conditions<-as.character(df$conditions)
df$participants<-as.character(df$participants)
...
The `means` column now shows all of the participant means. We then find the difference between each participant mean and the grand mean. These deviations are shown in the `diff` column. Then we square the deviations, and sum them up. 
...
summary.aovlist <- aov(scores~conditions + Error(participants/conditions),df[1:9,])
tidy_summary <- tidy(summary.aovlist)
tidy_summary_without_first_col <- tidy_summary[, -1] 

tidy_summary_without_first_col %>%
  kbl() %>%
  kable_styling(full_width = F, position = "left")
```
...
```{r}
summary_out <- summary(aov(scores~conditions + Error(participants/conditions),df[1:9,]))
...
### Repeated vs between-participants ANOVA
...
3. We will include a participant factor for the repeated-measures version. Here there are 10 participants, each contributing three scores, one each condition
4. For the between-participants design there are 30 different participants, each contributing one score in the condition they were assigned to (really the group).
...
participants <-as.factor(rep(1:10,3))
df<-data.frame(scores,conditions,participants)

between_out<-summary(aov(scores~conditions,df))
b_f[i] <- between_out[[1]]$`F value`[1]

within_out<-summary(aov(scores~conditions + Error(participants/conditions),df))
...
The repeated-measures $F$ distribution has degrees of freedom 2, and 18, for the numerator and denominator. There are 3 conditions, so df1 = 3-1 = 2. There are 10 participants, so df2 = (10-1)*(3-1) = 9*2 = 18. The critical value, assuming an alpha of 0.05 is 3.55. This means $F$ is 3.55 or larger 5% of the time under the null. 
...
```{r 8fpvaluedist, fig.cap="p-value distributions for a between and within-participants ANOVA"}
...
For the between participant design, there were `r length(b_p[b_p<.05])` experiments with a $p$ less than 0.05, or `r length(b_p[b_p<.05])/1000` of experiments were "significant", with alpha=.05.

For the within participant design, there were `r length(w_p[w_p<.05])` experiments with a $p$ less than 0.05, or `r length(w_p[w_p<.05])/1000` of experiments were "significant", with alpha=.05.
...
```{r 8pdistwithin, fig.cap="p-value distribution for within-participants design with n= 30"}
...
participants <-as.factor(rep(1:30,3))
df<-data.frame(scores,conditions,participants)
...
Wowsers! Look at that. When we ran 30 participants in the repeated measures design almost all of the $p$-values were less than .05. There were `r length(w_p[w_p<.05])` experiments with a $p$ less than 0.05, or `r length(w_p[w_p<.05])/1000` of experiments were "significant", with alpha=.05. That's huge! If we ran the repeated measures design, we would almost always detect the true difference when it is there. This is why the repeated measures design can be more sensitive than the between-participants design. 
...
## get participant mean RTs

participant_means <- exp1_data %>%
                 filter(Order==1, Correct==1, PureRTs<5000) %>%
                 dplyr::group_by(Subject, Block, Stimulus) %>%
                 dplyr::summarise(mean_rt = mean(PureRTs))

participant_means$Subject<-as.factor(participant_means$Subject)
participant_means$Block<-as.factor(participant_means$Block)
participant_means$Stimulus<-as.factor(participant_means$Stimulus)

## get condition mean RTs

plot_means <- participant_means %>%
...
visible_means<- participant_means %>%
                 filter(Block=="Visible keyboard")


s_out.aovlist <- aov(mean_rt~Stimulus + Error (Subject/Stimulus),visible_means)
tidy_s_out <- tidy(s_out.aovlist)
tidy_s_out_without_first_col <- tidy_s_out[, -1] 
tidy_s_out_without_first_col %>%
  kbl() %>%
  kable_styling(full_width = F, position = "left")
